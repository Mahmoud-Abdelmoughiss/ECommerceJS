...
//set categories in select Options
var options = '';
...
document.getElementById('categories').onchange = function(){
    document.getElementById('CurrentSelectedCats').innerHTML = this.value.toUpperCase();
};
...
document.getElementById('categories').addEventListener('change',function(){
   var catSelected = document.getElementById('categories').value;
   var catNumber = 0; 
   for(var i = 0 ;i < categories.length ; i++){
       if(catSelected==categories[i]){
           catNumber = i;
       }
   }
   // modify random list to hold products in selected category
   randomList = [];
   for(var i = 0 ;i<10 ;i++){
       randomList[randomList.length] = (catNumber*10)+i;
   }
   ...
   // display products in randomList
    ...
    document.getElementById('pageRefresh').style.display = 'block';
});
// ################# filter By Price ###########################
document.getElementById('priceRange').addEventListener('input',function(){
    var rangeSelected = this.value;
    randomList = [];
    for(var i = 0 ; i< productPrice.length ;i++){
        if(rangeSelected>productPrice[i]){
...
// document.getElementById('disposeSearch').addEventListener('click',function(){
//     document.getElementById('searchInput').value = '';
// });

//  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ search By Name -----------------------------
 document.getElementById('searchForProduct').addEventListener('input',function(){
     randomList = [];
    var inputSearchValue ='';
     inputSearchValue = this.value;
    inputSearchValue = inputSearchValue.toLowerCase();
    var pattern = new RegExp(inputSearchValue);
    for(var i = 0; i<productName.length ;i++){
        var test = pattern.test(productName[i].toLowerCase());
        if(test){
            randomList[randomList.length] = i;
        }
        console.log(test);
   ...
   // display products in randomList
    ...
    //     document.getElementById('searchResultText').innerHTML = 'Here Are The Results';
    // }else{
    //     document.getElementById('searchResultText').innerHTML = 'No Results Found!';
    // }
...
//         for(var i =0 ;i<52;i++){
//             document.getElementsByClassName('itemSearch')[i].style.display ='none';
//         }
//     }
// });
...
valueSelected.innerHTML = range.value+" L.E";
